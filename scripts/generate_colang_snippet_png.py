...
OUTPUT_PATH = "/Users/devanshwadhwani/Desktop/HoneyLLM2/submissions/assets/colang_rule_snippet.png"
...
# Header Title & Icon
title_text = "rules/block_sim_impersonation.co"
...
lines = [
    [
        ("01", (100, 116, 139)),
        ("  # Autonomous Synthesized Rail (Extracted from Honeypot Trace)", (148, 163, 184))
    ],
    [
        ("02", (100, 116, 139)),
        ("  define flow ", (244, 63, 94)), # Rose keyword
        ("block_sim_impersonation", (251, 191, 36)) # Amber flow name
    ],
    [
        ("03", (100, 116, 139)),
        ("    user ", (56, 189, 248)), # Sky blue role
        ("asks to bypass SIM verification with auditor token", (167, 243, 208)) # Mint green
    ],
    [
        ("04", (100, 116, 139)),
        ("    $is_safe", (147, 197, 253)), # Light blue variable
        (" = ", (226, 232, 240)),
        ("execute ", (244, 63, 94)), # Rose keyword
        ("check_custom_llama_guard()", (251, 191, 36)) # Amber function
    ],
    [
        ("05", (100, 116, 139)),
        ("    when not ", (244, 63, 94)), # Rose keyword
        ("$is_safe", (147, 197, 253)) # Variable
    ],
    [
        ("06", (100, 116, 139)),
        ("      bot ", (56, 189, 248)), # Role
        ("redirect to mirror_maze_sandbox(:9100)", (252, 165, 165)) # Soft coral target
    ],
    [
        ("07", (100, 116, 139)),
...
print(f"Colang rule snippet PNG successfully created at: {OUTPUT_PATH}")
